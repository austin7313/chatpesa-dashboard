      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div>
          <span style={{
            ...
            background: apiOnline ? "green" : "red", marginRight: 8
          }} />
          ...
          <tbody>
            {filteredOrders.length === 0 ? (
              ...
              <tr key={order.id} style={{ cursor: "pointer" }} onClick={() => setSelectedOrder(order)}>
                ...
                <td style={{ padding: 12 }}>{new Date(order.paid_at || order.created_at).toLocaleString()}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {selectedOrder && (
        <div onClick={() => setSelectedOrder(null)} style={{
          ...
          <div onClick={(e) => e.stopPropagation()} style={{
            background: "#fff", padding: 24, borderRadius: 12, width: "90%", maxWidth: 400
          }}>
            ...
            <p><b>M-Pesa Name:</b> {selectedOrder.mpesa_name || "-"}</p>
            <p><b>Time:</b> {new Date(selectedOrder.paid_at || selectedOrder.created_at).toLocaleString()}</p>
