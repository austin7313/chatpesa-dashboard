...
export default function App() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (data.status === "ok") { setOrders(data.orders || []); setApiOnline(true); }
  ...
  return (
    <div style={{padding:24,fontFamily:"Inter,Arial,sans-serif"}}>
      ...
      <input type="text" placeholder="Search by Order ID or Name..." value={search} onChange={(e)=>setSearch(e.target.value)}
            ...
            {filteredOrders.length===0?(
              ...
              filteredOrders.map(o=>(
                <tr key={o.id} style={{cursor:"pointer"}} onClick={()=>setSelectedOrder(o)}>
                  ...
                  <td style={{padding:12,color:getStatusColor(o.status),fontWeight:"bold"}}>{o.status}</td>
